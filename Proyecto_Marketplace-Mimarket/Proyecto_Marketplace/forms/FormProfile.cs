...
namespace Proyecto_Marketplace.forms
{
    public partial class FormProfile : Form
    {
        ...
        {
            // La carga de la imagen, incluyendo el placeholder, se centraliza en CargarImagenSegura
            string rutaRelativa = usuarioActual.obtenerRutaFotoPerfil();
            pictureBoxPerfil.Image = CargarImagenSegura(rutaRelativa);
        ...
        private Image CargarImagenSegura(string rutaRelativa)
        {
            ...
            // Lógica para cargar la imagen predeterminada (placeholder)
            try
            {
                // **CORRECCIÓN 1: Usamos el nombre de archivo y la ruta correcta**
                string placeholderPath = Path.Combine(Application.StartupPath, "media", "perfilPredeterminado.jpg");

                if (File.Exists(placeholderPath))
                {
                    // **CORRECCIÓN 2: Usamos FileStream para evitar el bloqueo del archivo**
            ...
            // Fallback si no se puede cargar ni siquiera el predeterminado
        ...
        private void boton_cambiarFoto_Click(object sender, EventArgs e)
        {
            ...
            if (dialogo.ShowDialog() == DialogResult.OK)
            {
                ...
                string rutaDestinoAbsoluta = Path.Combine(carpetaFotos, nombreArchivo);
                ...
                File.Copy(dialogo.FileName, rutaDestinoAbsoluta, true);

                // Para cargar, usamos la ruta absoluta
                using (var fs = new FileStream(rutaDestinoAbsoluta, FileMode.Open, FileAccess.Read))
                {
                    pictureBoxPerfil.Image = new Bitmap(fs);
                }

                //ALMACENA LA RUTA RELATIVA
                string rutaRelativa = Path.Combine("FotosPerfil", nombreArchivo);
                usuarioActual.cambiarRutaFotoPerfil(rutaRelativa);
        ...
        private void boton_eliminarFoto_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(rutaRelativa) && File.Exists(rutaAbsoluta))
            {
                ...
                if (resultado == DialogResult.Yes)
                {
                    if (!string.IsNullOrEmpty(rutaRelativa) && File.Exists(rutaAbsoluta))
                    {
                        ...
                        // ruta absoluta para eliminar
                        File.Delete(rutaAbsoluta);
                    }

                    usuarioActual.cambiarRutaFotoPerfil(""); //Guarda ruta vacía/nula
                    RepositorioUsuarios.ActualizarUsuario(usuarioActual);

                    // **CORRECCIÓN 3: Forzamos la carga del placeholder después de eliminar**
                    pictureBoxPerfil.Image = CargarImagenSegura("");
